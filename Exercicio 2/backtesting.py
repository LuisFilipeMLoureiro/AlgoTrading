...
class Strategy():
    ...
    def close(self):
        ...
        for instrument, position in self._position.items():
            if position != 0:
                if position > 0:
                    self.submit(self._id, Order(instrument, Order.S, position, 0))
                else:
                    self.submit(self._id, Order(instrument, Order.B, position, 0))
    ...
    def summary(self, tax=0.00024, fee=0):
        ...
        res += 'Max Trade P&L: {0:.2f}\n'.format(mp)
        res += 'Min Trade P&L: {0:.2f}\n'.format(md)
        ...
        if instrument in self.books:
            ...
            for id in self.listeners[instrument]:
                if id in self.strategies:
                    self.strategies[id].receive(event)
